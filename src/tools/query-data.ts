...
export function registerQueryData(server: McpServer, env?: QueryEnv) {
	const handler = createQueryDataHandler("FUNDING_GRAPH_DATA_DO", "funding_graph");

	server.registerTool(
		"funding_graph_query_data",
		{
			title: "Query Staged Funding Graph Data",
			description:
				"Query staged funding graph data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
			inputSchema: {
				data_access_id: z
					.string()
					.min(1)
